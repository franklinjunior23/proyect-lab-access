...
def _transceive(datos_tx, tx_last_bits=0):
    """
    Envía datos a la tarjeta y recibe la respuesta.
    tx_last_bits: bits a enviar del último byte (0 = byte completo, 7 = REQA/SHORT FRAME).
    ...
    _escribir_reg(_REG_BIT_FRAMING, 0x80 | tx_last_bits)  # StartSend + bits del último byte
...
def detectar_tarjeta():
    ...
    """
    ok, _ = _transceive([_PICC_REQA], tx_last_bits=7)  # REQA es SHORT FRAME: 7 bits
    return ok
